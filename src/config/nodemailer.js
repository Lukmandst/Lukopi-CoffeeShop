...
const sendConfirmationEmail = async (name, email, confirmationCode) => {
  try {
    ...
    let mailOptions = {
      ...
      html: `<h2>Lukopi Email Confirmation</h2>
      <div>
      <h3>Hi, ${name}</h3>
...
const sendConfirmationPayment = async (
  name,
  email,
  items,
  totalPrice,
  ...
  <h2> <a href=${
  ...
  }/auth/payment/${token}> Click here to confirm your payment</a></h2>
    </div>`;
    if (payMethod === "cash on delivery") {
     ...
     <h4> <a href=${
       process.env.CLIENT_URL
     }/auth/payment/${token}> Click here to confirm your order</a></h4>
      </div>`;
    }
...
const sendPasswordConfirmation = async (name, email, confirmCode) => {
  try {
    ...
    let html = `<h2>Lukipay Forgot Password Confirmation</h2>
    <div>
    <h3>Hi!</h3>
